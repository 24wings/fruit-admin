...
export class AppFruitService {
  ...
  private getRecordApi = '/fruit-client.getRecord.get';
  // 导出excel表
  ...
  exportToExcel(_headers: string[], rows: [string[]]) {
    return this.api.Post('/fruit-client.toExcel.post', {});

  }
  getRecordByTimeseg(startDt: number, endDt: number, userId: string) {
    return this.api.Get(this.getRecordApi + `?adminId=${this.adminId}&startDt=${startDt}&endDt=${endDt}&userId=${userId}`)
  }
